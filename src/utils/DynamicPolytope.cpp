...
void DynamicPolytope::addContactWrenchCone(const mc_rbdyn::Robot & robot, const std::string & surfaceName, double friction)
{
    ...
    std::pair<std::pair<double, double>, sva::PTransformd> contactSurface(std::pair<double, double>(newContact.halfLength(), newContact.halfWidth()), newContact.surfacePose());
    // XXX check if normal to compute generators matrix for several contacts? makes sense only if R-rep, not if V-rep and bounded polytopes
    auto com = robot.com();
    // compute 6d generators matrix for 6d cone of this contact
...
void DynamicPolytope::updateTriangles(const boost::shared_ptr<Polytope_Rn> & polytope, std::vector<std::array<Eigen::Vector3d, 3>> & resultTriangles)
{
    // XXX mutex?
    // XXX CAREFUL here we fill a triangles list: this assumes we are in 3d (because each facet has dim vertices in a polytope)
    // BUT this means if we actually manipulate a 6d space the faces will be hexagons? can we assume the generators are made of 2 3d matrices?
    resultTriangles.clear();
    // DoubleDescriptionFromGenerators::Compute(polytope);
    // Assuming the given polytope is already computed, get the generators for each facet, then use their coordinates to create the faces in mc_rtc format
	...
	for (halfSpaceIter.begin(); halfSpaceIter.end() != true; halfSpaceIter.next())
    {
        for (generatorIter.begin(); generatorIter.end() != true; generatorIter.next())
        {
            ...
            for (size_t i = 0; i < generatorIter.current()->numberOfFacets(); i++)
            {
                if (generatorIter.current()->getFacet(i) == halfSpaceIter.current())
                {
                    ...
                    // TODO build vector from loop
                    generatorIter.current()->getCoordinate()
                }
		...
		Eigen::Vector3d v1(face->get_vertex1()->get_coordinates()), v2(face->get_vertex2()->get_coordinates()), v3(face->get_vertex3()->get_coordinates());
		
		if (((v2-v1).cross(v3-v1)).dot(face->get_normal()) > 0.0){
			resultTriangles.push_back({v1, v2, v3});
		}
		else{
			resultTriangles.push_back({v1, v3, v2});
		}
	}
...
void DynamicPolytope::addToGUI(mc_rtc::gui::StateBuilder & gui, std::vector<std::string> category = {"DynamicPolytopes"})
{
    // also give polytope gui config
}